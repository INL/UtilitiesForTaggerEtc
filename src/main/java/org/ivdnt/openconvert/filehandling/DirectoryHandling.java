...
public class DirectoryHandling {
	...
	/**
	 * Difference with previous: this creates subdirectories mirroring the source
	 * directory
	 *
	 * @param p
	 ...
	 * @param fileFilter
	 */
	public static void traverseDirectory(FileInputOutputProcess p, File currentDirectory, File outputDirectory, FileFilter fileFilter) {
		if (usePathHandler) {
			try {
		...
		if (selectedFiles != null) {
			...
			for (File f : selectedFiles) {
				if (f.isDirectory()) {
					...
					traverseDirectory(p, new File(currentDirectory, f.getName()), outputSubdirectory, fileFilter);
				} else {
					try {
	...
	public static void traverseDirectory(FileInputOutputProcess p, String folderName, String outputDirectory) {
		File f = new File(folderName);
